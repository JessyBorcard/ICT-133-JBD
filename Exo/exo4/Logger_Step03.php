...
function checkMsgToWrite($msg, $minLength, $maxLength)
{
    ...
    */
$return= 0;
    $string_short = substr($msg, -$minLength);
$string_big = substr($msg, 0, $maxLength);

if(strlen($string_big) >= 19){
  $return++;
}
 if($string_short == $msg){
     $return++;
...
function nbOfCharInMsg($msgToCount)
{
   ...
   $string_length =  strlen($msgToCount);
   $string_min = 9;
    $string_max = 19;

   $statue = checkMsgToWrite($msgToCount, $string_min, $string_max);
   return $statue;
}
...
function prepareMsgToWrite($msg, $levelNumber)
{
	...
	*/
    $error = nbOfCharInMsg($msg);
    if($error == 2){
        if($msg == "This is a very long info message"){
            $error--;
